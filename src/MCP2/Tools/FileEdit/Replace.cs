...
namespace MCP2.Tools.FileEdit
{
    ...
    /// </summary>
    public class Replace : ITool
    {
        public string Name => "replace";
        public string Description => "Replace a single line in a file. Out-of-range line_number returns error. WARNING: Line numbers shift after edits — edit bottom-up or use batch_edit.";
        
        public ToolParamList Params => new ToolParamList()
            .String("path", "Full path to the file", required: true)
            .Int("line_number", "Line number to replace (1-based)", required: true)
            .String("new_content", "New content for the line", required: true)
        ...
        public ToolResult Execute(JObject args)
        {
            try
            {
                ...
                string note = FileOperations.EditLine(path, lineNumber, newContent);

                if (note != null)
                    return ToolResult.Success(string.Format("{0}{1}", note, backupInfo));
                
                return ToolResult.Success(string.Format(
                    "Line {0} replaced successfully.{1}", lineNumber, backupInfo));
            }
            catch (Exception ex)
            {
